...
- Never add code comments unless explicitly asked to do so.
- Never run tests unless explicitly asked to do so.
- Never run lint or formatters unless explicitly asked to do so.
- Never start Docker (or bring up docker-compose services) unless explicitly asked to do so.
- Never commit unless explicitly asked to do so.
- If verification would require any of the above, make the code change and tell me what to run instead.

## Debugging and research
- When you make a truth claim, always cite a specific file and line number as proof _or_ admit it's an educated guess.
- When possible, prove your truth claims with code examples the user can read and run.
